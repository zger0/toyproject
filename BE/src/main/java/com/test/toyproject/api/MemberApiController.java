...
public class MemberApiController {
    ...
    public List<MemberDTO> findByMembers(){
        ...
        List<MemberDTO> collect = findMembers.stream()
                .map(m -> new MemberDTO(m.getId(), m.getMemberId(), m.getMemberName(), m.getEmail()
    ...
    public MemberDTO findByMember(@PathVariable("id") Long id){
        ...
        return new MemberDTO(member.getId(), member.getMemberId(), member.getMemberName(), member.getEmail()
    ...
    public MemberDTO updateMember(@PathVariable("id") Long id, @RequestBody Member member){
        ...
        return new MemberDTO(m.getId(), m.getMemberId(), m.getMemberName(), m.getEmail()
                                        , m.getAddres(), m.getPhone(), m.getCreateDate());
    }
